...
const getTokenFrom = request => { // Alustetaan muuttuja "getTokenFrom", joka suorittaa {...} sisällä olevat asiat, kun kyseiseen funktioon viitataan.
  // Olemme Postmanin kautta luoneet erikseen => "Headers" osuuteen uuden arvon, nimeltään "Authorization", joka saa arvoksi => "bearer your_token_value_here".
  const authorization = request.get('authorization') // Alustetaan muuttuja "authorization", joka hakee kyseisen arvon Postmanin kautta.
  if (authorization && authorization.toLowerCase().startsWith('bearer ')) { // Jos kyseinen if-ehto toteutuu, niin funktio suorittaa alla olevan asian, ja muussa tapauksessa palauttaa => "return null".
    return authorization.substring(7) // Jos if-ehto toteutuu, niin suoritetaan kyseinen funktio.
  }
  return null // Jos if-ehto ei toteudu, niin suoritetaan kyseinen funktio.
}
...
blogRouter.post('/', async (request, response) => { // Funktion "post(...)" sisällä hyödynnetään myös "async/await" metodia!
  ...
  // Funktio "getTokenFrom" siis eristää tokenin headerista authorization ja sen oikeellisuus tarkistetaan "jwt.verity" funktion avulla.
  const token = getTokenFrom(request) // Alustetaan muuttuja "token", joka suorittaa kyseisen funktion "getTokenFrom(...)" ja palauttaa sen funktion kautta tulevan arvon takaisin "token" muuttujan alle.
  ...
  if (!token || !decodedToken.id) { // Jos kyseinen if-ehto toteutuu, niin suoritetaan {...} sisällä oleva asia. Muussa tapauksessa sovellus jatkaa eteenpäin.
  return response.status(401).json({ errorMessa: 'Your token is either missing or is invalid, please try again! :)' }) // Palautetaan pyyntöön takaisin "401 Unauthorized":in statuskoodi sekä kyseisen objektin arvo käyttäjälle.
}
