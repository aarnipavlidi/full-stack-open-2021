...
const mostLikes = (randomVariable) => { // Alustetaan muuttuja "mostLikes", kyseinen funktio ottaa myös käyttöönsä parametrin "randomVariable" arvon. Muuttujan arvon voi vaihtaa mihin tahansa halutessaan!

  let map = {} // Alustetaan muuttuja "map", joka toimii uutena tyhjänä taulukkona. Tämän muuttujan alle kerätään uudet arvot, jonne ilmestyy x henkilön nimi ja kuinka monta kertaa kyseinen henkilö saanut tykkäyksiä "blogValues" taulukosta yhteensä.
  let authorName = 0 // Alustetaan muuttuja "authorName", joka saa väliaikaisesti arvon nolla (0).
  let authorLikes = 0 // Alustetaan muuttuja "authorLikes", joka saa väliaikaisesti arvon nolla (0).

  for (let entry of randomVariable) { // Käytetään "for" funktiota, joka suorittaa yhteensä kolme (3) kierrosta, koska muuttujassa "randomVariable" (eli "blogValues") on myös kolme (3) elementtiä.
    if (map[entry.author] == null) {  // Jos if-ehto toteutuu eli "map[entry.author]" on yhtä kuin "null", niin suoritetaan {...} sisällä oleva asia.
      map[entry.author] = 0 // Muuttuja "map[entry.author]" saa arvoksi nolla (0).
    } // Jos if-ehto ei toteudu eli "map[entry.author]" on muuta kuin "null", niin suoritetaan alla oleva asia.
    map[entry.author]+= entry.likes // Kyseisen funktion avulla lisätään sen hetkinen "entry.likes" muuttujan arvo yhteen => "map[entry.author]" kanssa.

    // Jos if-ehto toteutuu eli muuttuja => "map[entry.author]" on suurempi kuin "authorLikes" muuttujan arvo, niin suoritetaan {...} sisällä olevat asiat.
    if (map[entry.author] >  authorLikes) {
      // Ensimmäisessä kierroksessa (for loop funktio), muuttujat saavat seuraavat arvot:
      // "map" on yhtä kuin "{'Edsger W. Dijkstra': 7}"
      // "entry.author" on yhtä kuin "Edsger W. Dijkstra"
      // "map[entry.author]" on yhtä kuin "7"
      authorName = entry.author // Muuttuja "authorName" saa sen hetkisen "entry.author" muuttujan arvon.
      authorLikes = map[entry.author] // Muuttuja "authorLikes" saa sen hetkisen "map[entry.author]" muuttujan arvon.
    }
  }

  const newValue = { // Alustetaan muuttuja "newValue", joka saa "author" ja "likes" objektit käyttöönsä.
    author: authorName, // eli "newValue.author" on yhtä kuin "authorName".
    likes: authorLikes // eli "newValue.likes" on yhtä kuin "authorLikes".
  }

  return newValue // Palautetaan kyseisen muuttujan "newValue" arvo, kun testiä suoritetaan.
}

module.exports = { // Viedään kyseiset muuttujat "dummy", "totalLikes", "favoriteBlog", "mostBlogs" ja "mostLikes" erillisenä moduulina testiä varten.
  dummy,
  totalLikes,
  favoriteBlog,
  mostBlogs,
