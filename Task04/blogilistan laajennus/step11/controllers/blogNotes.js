...
// Olemme alustaneet alla olevan muuttujan, koska haluamme, että sovellus käyttää => "middleware.tokenExtractor" ja "middleware.userExtractor" muuttujia,
// "toistaiseksi" ainoastaan, kun halutaan lisää uusi "blogin" arvo tietokantaan tai poistaa kyseinen arvo tietokannasta!
...
blogRouter.post('/', middleware.tokenExtractor, middleware.userExtractor, async (request, response) => { // Funktion "post(...)" sisällä hyödynnetään myös "async/await" metodia!
  ...
  // Aina kun sovellukseen tehdään pyyntö tähän kyseiseen funktioon eli post(...) (osoitteeseen => "http://localhost:3000/api/blogs"),
  // niin haetaan alla olevien kolmen (3) muuttujien arvot erikseen => "middleware.js" tiedostosta. Sieltä löytyy muuttuja =>
  // "tokenExtractor", joka kertoo pyynnön kautta tulevan "token" ja "id" arvon sekä muuttuja => "userExtractor", joka kertoo
  // pyynnön kautta tulevan "user" arvon. Tämä tarkoittaa, että aina kun kyseinen funktio suoritetaan, niin me tiedetään,
  // että kuka kyseisen pyynnön suorittaa => kolmen (3) muuttujan arvon avulla! :)
  const personName = request.user // Alustetaan muuttuja "userName", joka on yhtä kuin "request.user" muuttujan arvo.
  console.log(`Following person called "${personName}" is trying add new value to the database! :)`) // Tulostaa kyseisen tekstin terminaaliin näkyviin ja näyttää lopuksi pyynnön kautta tulevan muuttujan arvon.

  if (!request.token || !request.id) { // Jos kyseinen if-ehto toteutuu, niin suoritetaan {...} sisällä oleva asia. Muussa tapauksessa sovellus jatkaa eteenpäin.
...
// ALla oleva muuttuja, siis etsii tietokannasta => "blogilista.users" käyttäjätunnusta, jonka "id":n arvo on yhtä kuin => "request.id":n arvo.
const getUser = await User.findById(request.id) // Alustetaan muuttuja "getUser", joka suorittaa kyseisen funktion. Sovellus jatkaa eteenpäin vasta (await:in takia) kunnes funktio palauttanut arvon takaisin muuttujan alle.
...
blogRouter.delete('/:id', middleware.tokenExtractor, middleware.userExtractor, async (request, response) => { // Funktion "delete(...)" sisällä hyödynnetään myös "async/await" metodia!
  ...
  // Aina kun sovellukseen tehdään pyyntö tähän kyseiseen funktioon eli delete(...) (osoitteeseen => "http://localhost:3000/api/blogs/:id"),
  // niin haetaan alla olevien kolmen (3) muuttujien arvot erikseen => "middleware.js" tiedostosta. Sieltä löytyy muuttuja =>
  // "tokenExtractor", joka kertoo pyynnön kautta tulevan "token" ja "id" arvon sekä muuttuja => "userExtractor", joka kertoo
  // pyynnön kautta tulevan "user" arvon. Tämä tarkoittaa, että aina kun kyseinen funktio suoritetaan, niin me tiedetään,
  // että kuka kyseisen pyynnön suorittaa => kolmen (3) muuttujan arvon avulla! :)
  const personName = request.user // Alustetaan muuttuja "userName", joka on yhtä kuin "request.user" muuttujan arvo.
  console.log(`Following person called "${personName}" is trying to delete value from the database! :)`) // Tulostaa kyseisen tekstin terminaaliin näkyviin ja näyttää lopuksi pyynnön kautta tulevan muuttujan arvon.

  if (!request.token || !request.id) { // Jos kyseinen if-ehto toteutuu, niin suoritetaan {...} sisällä oleva asia. Muussa tapauksessa sovellus jatkaa eteenpäin.
    return response.status(401).json({ errorMessage: 'Your token is either missing or is invalid, please try again! :)' }) // Palautetaan pyyntöön takaisin "401 Unauthorized":in statuskoodi sekä kyseisen objektin arvo käyttäjälle.
  }

  if (getValues.user.toString() === request.id.toString()) { // Jos kyseinen if-ehto toteutuu, niin suoritetaan {...} sisällä olevat asiat. Muussa tapauksessa sovellus jatkaa eteenpäin.
