...
const userData = mongoose.Schema({ // Alustetaan muuttuja "userData", joka hyödyntää "mongoose.Schema({...})" funktiota. Sen avulla määritellään miten me tallennetaan (missä rungossa) data tietokantaan.
  username: { // Tietokannasta löytyy "username" objekti, joka hyödyntää {...} sisällä olevia asetuksia.
    ...
    required: true, // Jotta voidaan suorittaa esim. POST-pyyntö onnistuneesti, niin kyseinen objekti ei saa olla tyhjä arvo, muuten tulee erroria!
    min: 3 // Jotta voidaan suorittaa esim. POST-pyyntö onnistuneesti, niin kyseisen objektin pituus (length) täytyy olla vähintään kolme (3) kirjainta, muuten tulee erroria!
  },
  name: String, // Tietokannasta löytyy "name" objekti, joka käyttää "String" datatyyppiä.
  passwordHash: { // Tietokannasta löytyy "passwordHash" objekti, joka hyödyntää {...} sisällä olevia asetuksia.
    type: String, // Annetaan "passwordHash" objektille tyypiksi "String" datatyyppi.
    required: true, // Jotta voidaan suorittaa esim. POST-pyyntö onnistuneesti, niin kyseinen objekti ei saa olla tyhjä arvo, muuten tulee erroria!
    min: 3 // Jotta voidaan suorittaa esim. POST-pyyntö onnistuneesti, niin kyseisen objektin pituus (length) täytyy olla vähintään kolme (3) kirjainta, muuten tulee erroria!
  },
  content: [ // Tietokannasta löytyy "content" objekti, joka hyödyntää {...} sisällä olevia asetuksia.
    {
      ...
      ref: 'Blog' // Tämä tarkoittaa, että yllä oleva "ObjectId" viittaa "User"-tyyppisiin dokumentteihin (Collection).
    }
  ],
})
