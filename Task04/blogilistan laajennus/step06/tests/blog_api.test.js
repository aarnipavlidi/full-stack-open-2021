...
// Käytetään "test(...)" funktiota, joka suorittaa {...} sisällä olevat asiat. Testi myös hyödyntää "async/await" metodia!
test('Checking, if deleting value is successful from the database, which should return us "204" status code! :)', async() => { // Tulostetaan kyseinen teksti terminaaliin näkyviin, kun testiä suoritetaan.

  // Alustetaan muuttuja "getValues", joka odottaa (await) kunnes => "helper.blogsInDatabase()" on suoritettu ja palauttaa takaisin vastauksen.
  // Ota myös huomioon, että olemme aikaisemmin alustaneet muuttujan "helper", joka hyödyntää "test_helper.js" tiedostoa, mistä löytyy funktio => "blogsInDatabase()".
  const getValues = await helper.blogsInDatabase() // Kun "getValues" muuttuja saa arvon käytettäväksi, niin suoritetaan alla oleva funktio.
  const deleteValue = getValues[1] // Alustetaan muuttuja "deleteValue", joka on yhtä kuin "getValues[1]" muuttujan arvo. Tämän avulla siis määritellään mikä arvo poistetaan tietokannasta testin aikana.

  await api // Testi pysähtyy "väliaikaisesti" kunnes on suorittanut kaikki kaksi (2) olevaa asiaa ja siirtyy eteenpäin koodissa.
    .delete(`/api/blogs/${deleteValue.id}`) // DELETE-pyyntö ohjautuu osoitteeseen => "http://localhost:3001/api/blogs/:id"
    .expect(204) // Testi olettaa, että vastauksena saadaan "204 (No Content)":n statuskoodi. Jos saamme muun arvon kuin kyseisen, niin testi epäonnistuu!

  // Alustetaan muuttuja "valuesAfter", joka odottaa (await) kunnes => "helper.blogsInDatabase()" on suoritettu ja palauttaa takaisin vastauksen.
  // Ota myös huomioon, että olemme aikaisemmin alustaneet muuttujan "helper", joka hyödyntää "test_helper.js" tiedostoa, mistä löytyy funktio => "blogsInDatabase()".
  const valuesAfter = await helper.blogsInDatabase() // Kun "valuesAfter" muuttuja saa arvon käytettäväksi, niin suoritetaan alla oleva funktio.
  expect(valuesAfter).toHaveLength(helper.defaultValues.length - 1) // Testi olettaa, että muuttuja "valuesAfter" on yhtä kuin (pituuden osalta eli "length") muuttujan "helper.defaultValues" pituuden kanssa.

  const findAuthors = valuesAfter.map(result => result.author) // Alustetaan muuttuja "findAuthors", jonka avulla luodaan uusi taulukko "map(...)" funktion avulla, joka etsii kaikki "author" nimiset objektit tietokannasta.
  expect(findAuthors).not.toContain(deleteValue.author) // Testi olettaa, että muuttuja "findAuthors" ei saa sisältää samaa arvoa kuin "deleteValue.author" muuttujan sisältä.
  // Jos molemmat "expect()" funktiot toteutuvat oletetusti, niin testi menee onnistuneesti läpi. Muussa tapauksessa testi epäonnistuu ja tulee erroria terminaaliin näkyviin!
})

// Käytetään "test(...)" funktiota, joka suorittaa {...} sisällä olevat asiat. Testi myös hyödyntää "async/await" metodia!
test('Checking, if updating "likes" object value is successful on the database! :)', async() => { // Tulostetaan kyseinen teksti terminaaliin näkyviin, kun testiä suoritetaan.

  // Alustetaan muuttuja "getValues", joka odottaa (await) kunnes => "helper.blogsInDatabase()" on suoritettu ja palauttaa takaisin vastauksen.
  // Ota myös huomioon, että olemme aikaisemmin alustaneet muuttujan "helper", joka hyödyntää "test_helper.js" tiedostoa, mistä löytyy funktio => "blogsInDatabase()".
  const getValues = await helper.blogsInDatabase() // Kun "getValues" muuttuja saa arvon käytettäväksi, niin suoritetaan alla oleva funktio.
  const updateValue = getValues[getValues.length - 1] // Alustetaan muuttuja "updateValue", joka on yhtä kuin "getValues[getValues.length - 1]" muuttujan arvo. Tämän avulla siis määritellään mitä arvoa päivitetään tietokannasta testin aikana.

  const newValue = { // Alustetaan muuttuja "newValue", joka saa käyttöönsä alla olevan objektin.
    likes: '25' // newValue.likes on yhtä kuin kyseinen arvo.
  }

  await api // Testi pysähtyy "väliaikaisesti" kunnes on suorittanut kaikki kolme (3) olevaa asiaa ja siirtyy eteenpäin koodissa.
    .put(`/api/blogs/${updateValue.id}`) // PUT-pyyntö ohjautuu osoitteeseen => "http://localhost:3001/api/blogs/:id"
    .send(newValue) // Lähetetään osoitteeseen muuttujan "newValue" arvo.
    .expect(200) // Testi olettaa, essä vastauksena saadaan "200 OK":n statuskoodi. Jos saamme muun arvon kuin kyseisen, niin testi epäonnistuu!

  // Alustetaan muuttuja "valuesAfter", joka odottaa (await) kunnes => "helper.blogsInDatabase()" on suoritettu ja palauttaa takaisin vastauksen.
  // Ota myös huomioon, että olemme aikaisemmin alustaneet muuttujan "helper", joka hyödyntää "test_helper.js" tiedostoa, mistä löytyy funktio => "blogsInDatabase()".
  const valuesAfter = await helper.blogsInDatabase() // Kun "valuesAfter" muuttuja saa arvon käytettäväksi, niin suoritetaan alla oleva funktio.
  expect(valuesAfter[valuesAfter.length - 1].likes).toBe(25) // Testi olettaa, että muuttuja "valuesAfter[valuesAfter.length - 1].likes" on yhtä kuin "25" (eli "newValue.likes" muuttujan arvo). Jos funktio toteutuu oletetusti, niin testi menee onnistuneesti läpi. Muussa tapauksessa testi epäonnistuu ja tulee erroria terminaaliin näkyviin!
})
