...
// Alla oleva kirjasto otetaan käyttööön komennolla => "npm install --save mongoose-unique-validator".
...
mongoose.connect(database, { useNewUrlParser: true, useUnifiedTopology: true, useFindAndModify: false, useCreateIndex: true }) // Kyseisen funktion "mongoose.connect(...)" avulla voidaan yhdistää tietokantaan (MongoDB). Lisää infoa tästä @ https://mongoosejs.com/docs/connections.html
 ...
 const mongoFrame = new mongoose.Schema({ // Alustetaan muuttuja "mongoFrame", joka määrittelee miten me tallennetaan (missä rungossa) ne puhelinluettelon tietokantaan.
   name: { // Tietokannan kokoelmasta löytyy "name" muuttuja => puhelinluettelo.persons.name
     type: String, // Objektin "name" tyyppi on "String".
     minlength: 3, // Kun käyttäjä haluaa lisätä uuden henkilön luetteloon, niin "name" objektin arvo täytyy olla vähintään kolme (3) merkkiä.
     required: true, // Objekti "name" ei saa olla tyhjänä, muuten henkilöä ei viedä tietokantaan.
     unique: true // Objektin "name" täytyy olla uniikki eli mikäli tietokannasta löytyy jo entuudestaan samanlainen arvo, niin henkilöä ei viedä tietokantaan.
   },
   number: { // Tietokonnan kokoelmasta löytyy "number" muuttuja => puhelinluettelo.persons.number
     type: String, // Objektin "number" tyyppi on "String".
     minlength: 8, // Kun käyttäjä haluaa lisätä uuden henkilön luetteloon, niin "number" objektin arvo täytyy olla vähintään kahdeksan (8) merkkiä.
     required: true // Objekti "number" ei saa olla tyhjänä, muuten henkilöä ei viedä tietokantaan.
   }
 })

 mongoFrame.plugin(uniqueValidator) // Jos tätä ei ole, niin kun tulee errori nii tulee "MongoError: E11000 duplicate key error collection..."
