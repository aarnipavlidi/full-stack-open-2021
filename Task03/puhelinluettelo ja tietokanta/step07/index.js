...
program.post("/api/persons", (request, response, next) => { // Määritellään sovellukselle tapahtumankäsittelijä (event handler), joka hoitaa sovelluksen polkuun eli "/api/persons" tulevia "HTTP POST" pyyntöjä.
  ...
  if (!getValue.name || !getValue.number) { // Jos "getValue.name" tai "getValue.number" muuttujat ovat tyhjiä, niin if-ehto toteutuu ja suoritetaan {...} sisällä olevat asiat.
      console.log("No empty values! Please add either name or number and try again! :)") // Tulostaa kyseisen tekstin terminaaliin näkyviin ja palauttaa "400 Bad Request":in käyttäjälle sekä alla olevan tekstin näkyviin selaimen tai Postmanin kautta.
  ...
  buildData.save().then(showResult => { // "muuttuja.save()" eli "buildData.save()" funktio tallentaa muuttujan datan tietokantaan, jonka jälkeen suoritetaan .then({...}) funktio.
    response.json(showResult) // Palauttaa "showResult" muuttujan sisällä olevan datan takaisin JSON muodossa.
    console.log(showResult) // Tulostetaan "showResult" muuttujan arvo takaisin terminaaliin näkyviin.
  }) // Kun yritetään lisätä uutta henkilöä tietokantaan ja sen yhteydessä tulee jonkinlainen virhe (esim. nimi puuttuu tai liian vähän kirjaimia yms.), niin sovellus suorittaa alla olevan ".catch(...)" funktion.
  .catch(error => next(error)) // Parametrin "next" avulla funktio siirtää virheen käsittelyn "virheidenkäsittelymiddlewarelle" eli => "const errorHandler" muuttujan alle.
})
...
const errorHandler = (error, request , response, next) => { // Alustetaan muuttuja "errorHandler" (virheidenkäsittelymiddleware), joka saa neljä (4) erilaista parametriä käytettäväksi.
  ...
  if (error.name === "CastError") { // Jos if-ehto toteutuu eli "error.name" muuttujan arvo on yhtä kuin "CastError", niin suoritetaan {...} sisällä olevat asiat.
    console.log("Error happened while doing something, check Chrome or Postman for more info! :)") // Tulostaa kyseisen tekstin terminaaliin näkyviin ja palauttaa "400 Bad Request":in käyttäjälle sekä alla olevan tekstin näkyviin selaimen tai Postmanin kautta.
  ...
  } else if (error.name === "ValidationError") { // Jos if-ehto toteutuu eli "error.name" muuttujan arvo on yhtä kuin "ValidationError", niin suoritetaan {...} sisällä olevat asiat.
    console.log("Error happened while trying to add or update person to the database, check Chrome or Postman for more info! :)") // Tulostaa kyseisen tekstin terminaaliin näkyviin ja palauttaa "400 Bad Request":in käyttäjälle sekä alla olevan tekstin näkyviin selaimen tai Postmanin kautta.
