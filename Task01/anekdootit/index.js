...
const App = (props) => {
  ...
  const highestText = taulukko.indexOf(Math.max(...taulukko)) // Me tiedetään, että maxResult on yhtä kuin nolla (0)
  const highestValue = Math.max(...taulukko)
    ...
    <div>
      ...
      <div>
        ...
        <div>
          ...
          <h3>{props.anecdotes[highestText]}</h3> // Tulostaa tekstin muuttujasta "anecdotes", jossa [highestText] vastaa korkeinta arvoa taulukon sisällä eli taulukko = [0, 0, 0, 0, 0, 0] (oletus, kun käyttäjä avaa sivun ensimmäisen kerran).
          <h4>and this text has been voted for total of {highestValue} times!</h4> // Tulee näkyviin kyseinen teksti ja näyttää sen arvon (tekstiä mitä on äänestetty eniten).
        </div>
  </div>
)
}
