...
const resolvers = {
  Query: {
    ...
    // Kun suoritetaan "allBooks" query, niin sovellus etsii "books" muuttujan sisältä jokaisen arvon,
    // joka täsmää "args.author" muuttujan kanssa. HUOM! Oletuksena, että käyttäjä laittaa queryyn
    // parametrin arvon, koska mikäli kyseinen arvo puuttuu, niin query palauttaa automaattisesti
    // "books" muuttujan arvon takaisin käyttäjälle!
    allBooks: (root, args) => {
      // Jos alla oleva if-ehto toteutuu eli => "args.author" on yhtä kuin
      // "undefined" arvo, niin sovellus suorittaa {...} sisällä olevan
      // asian ja palauttaa takaisin käyttäjälle "books" muuttujan arvon.
      if (args.author === undefined) {
        return books
      // Muussa tapauksessa suoritetaan alla oleva ehto eli luodaan uusi taulukko
      // "filter(...)" funktion avulla sekä palautetaan jokainen arvo, jossa ehto
      // toteutuu eli => "results.author" on yhtä kuin "args.author" arvon kanssa.
      } else {
