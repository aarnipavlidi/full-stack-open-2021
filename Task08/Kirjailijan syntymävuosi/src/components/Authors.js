...
const Authors = ({ showAuthors, setNotification }) => {
  const [name, setName] = useState('') // Alustetaan muuttuja "name" tilaan, joka saa oletuksena arvon => ''.
  ...
  const submitAuthor = async (event) => {
    ...
    if (checkingAuthorValue === true) {
      ...
      setName('') // Kun uusi arvo on lisätty tietokantaan, niin tyhjennetään muuttujan arvo takaisin alkuperäiseen tilaan.
        ...
        <h2 className='text-center'>Update authors birthyear</h2>
          <form onSubmit={submitAuthor}>
            <div className='mb-3 col-sm-5'>
              <label for='FormInputOne' className='form-label'>Author:</label>
              <input
                value={name}
                onChange={({ target }) => setName(target.value)}
                className='form-control'
                id='FormInputOne'
                placeholder='Type which author you want to update...'
              />
            </div>
            <div className='mb-3 col-sm-5'>
