...
// vittaus. Komponentti saa myös käyttöönsä "showAuthors", "tokenStatus" ja "setNotification" parametrien arvot.
const Authors = ({ showAuthors, tokenStatus, setNotification }) => {
  ...
  const submitAuthor = async (event) => {
    ...
    if (checkingAuthorValue === true) {
      updateAuthorYear({ // Suoritetaan "updateAuthorYear(...)" funktio, joka saa käyttöönsä alla olevat muuttujat.
        variables: {
  ...
  // Alustetaan muuttuja "showEditAuthors", joka suorittaa {...} sisällä olevat asiat aina,
  // kun kyseiseen funktioon tehdään viittaus. Funktion tarkoituksena on siis tarkistaa, että
  // onko sovelluksen käyttäjä kirjautunut sisään. Mikäli hän on, niin renderöidään mahdollisuus
  // muokata nykyisten kirjailijoiden syntymävuosia. Muussa tapauksessa ei renderöidä mitään ja
  // ainoastaan lista tietokannassa olevista kirjailijoista näkyy takaisin käyttäjälle.
  const showEditAuthors = () => {
    // Jos alla oleva if-ehto toteutuu eli "tokenStatus" muuttuja on yhtä kuin
    // arvoa "null", niin funktio suorittaa {...} sisällä olevan asian.
    if (tokenStatus === null) {
      return null
    } else { // Muussa tapauksessa suoritetaan {...} sisällä olevat asiat takaisin käyttäjälle.
      return (
        <div className='container mt-3'>
          ...
          <form onSubmit={submitAuthor}>
            ...
            {born.toString().length < '4'
              ? <button className='btn btn-sm btn-outline-secondary' disabled>Choose author and give new birthyear</button>
  ...
  // Ota myös huomioon, että olemme lisänneet vielä "showEditAuthors(...)" funktion, joka renderöi elementin mikäli tietty ehto täyttyy!
  return (
    <div className='container mt-3'>
      <div>
