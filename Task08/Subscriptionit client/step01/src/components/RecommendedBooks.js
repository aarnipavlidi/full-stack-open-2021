...
// "showRecommendedBooks" ja "getUpdate" muuttujien arvot.
const RecommendedBooks = ({ showRecommendedBooks, getUpdate }) => {
  ...
  // Komponentti suorittaa kyseisen "useEffect(...)" funktion aina,
  // kun tapahtuu muutoksia "currentUserData" muuttujan osalta.
  // Ylhäällä on selitetty tarkemmin tämän funktion käyttötarkoitusta.
  useEffect(() => {
  ...
  // Komponentti suorittaa kyseisen "useEffect(...)" funktion aina,
  // kun tapahtuu muutoksia "getUpdate" muuttujan osalta.
  useEffect(() => {
    // Jos alla oleva if-ehto toteutuu eli "getUpdate" on muuta kuin => "null",
    // niin suoritetaan {...} sisällä olevat asiat. Muussa tapuksessa funktio
    // ei tee mitään, vaikka "getUpdate" muuttujassa tapahtuisi muutoksia.
    if (getUpdate !== null) {
      currentBooks({
        variables: { nameToSearch: currentGenre }
      })
