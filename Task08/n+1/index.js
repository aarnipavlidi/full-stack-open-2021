...
const DataLoader = require("dataloader") // Alustetaan muuttuja "DataLoader", joka ottaa käyttöön "dataloader":in nimisen kirjaston sovelluksen ajaksi.
...
// Tehtävää "8.26: n+1" varten olemme alustaneet erikseen kyseisen funktion.
mongoose.set('debug', true); // Alustetaan kyseinen funktio, joka siis tulosta terminaaliin kun suoritetaan esim. erilaisia querya palvelinta vastaan.
...
const resolvers = {
  Query: {
    ...
    allAuthors: (root, args, parent) => {
      // Tehtävään "8.26: n+1" varten koodia on muokattu, niin että lisätty queryyn uusi parametri
      // "parent", lisätty funktio nimeltään "authorLoader", joka suorittaa sen sisällä olevat asiat
      // kun funktioon tehdään viittaus. Loaderille pitää lopuksi myös asettaa "Promise.resolve"
      // funktio, joka siis muuttaa "result" datan promiseksi => "return Promise.resolve(result)".
      // Vinkkejä kyseiseen tehtävään löysin täältä: https://rahmanfadhil.com/graphql-dataloader/
      const authorLoader = new DataLoader((keys) => {
        ...
        return Promise.resolve(result) // Palauttaa takaisin "result" muutttujan datan promiseksi.
      })

      console.log("I am now showing data from 'allAuthors' query and using DataLoader to do so! :)") // Tulostaa kyseisen tekstin takaisin terminaaliin näkyviin.
      return authorLoader.load(parent) // Palauttaa takaisin kyseisen funktion.
    }
  },
