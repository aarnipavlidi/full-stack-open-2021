...
const App = () => { // Sovellus alkaa tästä...
  ...
  const addBlogValue = async (blogContent) => { // Alustetaan muuttuja "addBlogValue", joka suorittaa {...} sisällä olevat asiat.
    ...
    dispatch(showNotificationSuccess(`You have successfully added ${response.title} to the database! :)`)) // Sovellus suorittaa funktion "showNotificationSuccess(...)", joka saa kyseisen tekstin parametrin arvoksi.
  ...
  const updateBlogValue = async (blogContent) => { // Alustetaan muuttuja "updateBlogValue", joka suorittaa {...} sisällä olevat asiat.
    ...
    dispatch(showNotificationSuccess(`You have successfully added new like to the ${response.title}! :)`)) // Sovellus suorittaa funktion "showNotificationSuccess(...)", joka saa kyseisen tekstin parametrin arvoksi.
    setTimeout(() => { // Ajastin, joka muuttaa {...} sisällä olevien muuttujien arvot takaisin alkuperäiseen arvoon eli "null".
      console.log('Notification is now hidden from the user! :)') // Tulostetaan kyseinen teksti konsoliin näkyviin.
      dispatch(hideNotification()) // Sovellus suorittaa funktion "hideNotification()", joka piilottaa "popup" viestin pois näkyviltä.
    }, 3000) // Yllä olevat funktiot suoritetaan 3. sek kuluttua.
  ...
  const deleteBlogValue = async (currentContent) => { // Alustetaan muuttuja "deleteBlogValue", joka suorittaa {...} sisällä olevat asiat.
    ...
    try {
      if (window.confirm(`Are you sure you want to delete ${currentContent.title} from the database?`)) { // Jos käyttäjä vahvistaa "window.confirm(...)" funktion, niin sovellus suorittaa {...} sisällä olevat asiat.
        ...
        dispatch(showNotificationSuccess(`You have successfully deleted blog called ${currentContent.title} ! :)`)) // Sovellus suorittaa funktion "showNotificationSuccess(...)", joka saa kyseisen tekstin parametrin arvoksi.
        setTimeout(() => { // Ajastin, joka muuttaa {...} sisällä olevien muuttujien arvot takaisin alkuperäiseen arvoon eli "null".
          console.log('Notification is now hidden from the user! :)') // Tulostetaan kyseinen teksti konsoliin näkyviin.
          dispatch(hideNotification()) // Sovellus suorittaa funktion "hideNotification()", joka piilottaa "popup" viestin pois näkyviltä.
        }, 3000) // Yllä olevat funktiot suoritetaan 3. sek kuluttua.
      ...
      console.error(error) // Tulostaa konsoliin kyseisen muuttujan arvon näkyviin käyttäjälle.
      dispatch(showNotificationError(`There was a problem deleting ${currentContent.title} from the database. Please try again later! :)`)) // Sovellus suorittaa funktion "showNotificationError(...)", joka saa kyseisen tekstin parametrin arvoksi.
      setTimeout(() => { // Ajastin, joka muuttaa {...} sisällä olevien muuttujien arvot takaisin alkuperäiseen arvoon eli "null".
        console.log('Notification is now hidden from the user! :)') // Tulostetaan kyseinen teksti konsoliin näkyviin.
        dispatch(hideNotification()) // Sovellus suorittaa funktion "hideNotification()", joka piilottaa "popup" viestin pois näkyviltä.
      }, 3000) // Yllä olevat funktiot suoritetaan 3. sek kuluttua.
