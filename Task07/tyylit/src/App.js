...
import { BrowserRouter as Router, Switch, Route } from 'react-router-dom' // Alustetaan komponentti "BrowserRouter", joka nimetään uudestaan (as:in avaulla). Komponentti hyödyntää "react-router-dom" kirjastoa sovelluksen aikana.

import Blog from './components/Blog' // Sovellus hyödyntää "Blog" (Blog.js) nimistä komponenttia, joka sijaitsee => "./components/Blog".
import BlogByID from './components/BlogByID' // Sovellus hyödyntää "BlogByID" (BlogByID:js) nimistä komponettia, joka sijaitsee => "./components/BlogByID".
...
import MenuList from './components/MenuList' // Sovellus hyödyntää "MenuList" (MenuList.js) nimistä komponenttia, joka sijaitsee => "./components/MenuList".
import Users from './components/Users' // Sovellus hyödyntää "Users" (Users.js) nimistä komponenttia, joka sijaitsee => "./components/Users".
import UsersByID from './components/UsersByID' // Sovellus hyödyntää "UsersByID" (UsersByID.js) nimistä komponenttia, joka sijaitsee => "./components/UsersByID".
...
const App = () => { // Sovellus alkaa tästä...
    ...
    <div className='jumbotron vertical-center'>
      <div className='container mt-5'>
        <div className='custom-background'>
          ...
          <form className='pt-3 ps-3 pe-3' onSubmit={handleLogin}>
            ...
            <button className='btn btn-outline-secondary mb-3' type="submit">login</button>
          </form>
        </div>
      </div>
    </div>
  ...
  const blogForm = () => {
    ...
    // Alla olevan "return (...)" sisältä löytyy <Switch>...</Switch> eli kaikki sisältö mitkä ovat
    // tämän sisällä renderöidyy sen mukaan, mitä käyttäjä painaa => "<MenuList />" (komponentti).
    // Kaikki muut siis näkyvät jatkuvasti käyttäjälle eli niitä ei "renderöidä uudelleen".
    return (
      <div>
        <div className='container text-center'>
          <p>Welcome to the application {userInfo.name}, you are now logged in to the bloglist! :) <button className='btn btn-outline-secondary ms-3' id='logout-button' onClick={handleLogout}>Logout</button></p>
        </div>
        <Router>
          <div style={navbarCSS}>
            <MenuList />
          ...
          <Switch>
            ...
            <Route path='/blogs/:id'>
              <BlogByID getCurrentBlogs={blogs} getCurrentUser={userInfo} />
            </Route>
            <Route path='/'>
              <div className='container' style={hideBlogForm}>
                <button className='btn btn-outline-secondary' id='create-blog-button' onClick={() => setBlogFormVisible(true)}>Create new blog</button>
              </div>
              <div className='container' style={showBlogForm}>
                <BlogForm />
                <button className='btn btn-outline-secondary mt-2' onClick={() => setBlogFormVisible(false)}>Cancel</button>
              </div>
              <div id="blogValues">
                {blogs.sort((a, b) => b.likes - a.likes).map(results =>
                  <Blog key={results.id} getBlogValues={results} />
                )}
              </div>
            </Route>
          </Switch>
  ...
  return (
    <div className='blogs'>
      {userInfo === null ? <div className='container text-center mt-5'><h1>Welcome to Bloglist, please log in! :)</h1></div> : <div className='container text-center mt-5'><h1>Bloglist</h1></div>}
      <Notification />
