...
const CreateNew = (props) => { // Alustetaan komponentti "CreateNew", joka suorittaa {...} sisällä olevat asiat.
  ...
  // Alustetaan muuttuja "resetCreateValueForm", joka suorittaa {...} sisällä olevat asiat,
  // aina kun kyseiseen funktioon tehdään viittaus. Kun käyttäjä haluaa poistaa sen hetkiset
  // arvot input:in kentistä, niin "resetInputValue()" funktion avulla tyhjennetään kaikkien
  // kolmen (3) muuttujan arvot eli ne pääsevät alkuperäiseen tilaan.
  const resetCreateValueForm = () => {
    getNewValueContent.resetInputValue()
    getNewValueAuthor.resetInputValue()
    getNewValueInfo.resetInputValue()
  }
    ...
    <div>
      ...
      <form>
        ...
        <div>
          ...
          <button type='button' onClick={resetCreateValueForm}> Reset</button>
        </div>
      </form>
    </div>
  )
