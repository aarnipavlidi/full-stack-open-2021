...
const Users = () => {
  ...
  // Kun käyttäjä klikkaa tiettyä käyttäjää ("<Link>...</Link"), niin sovellus
  // ohjautuu kyseisen käyttäjän id:n arvoon (esim. "users/609a60df12b0752498de41df"),
  // joka taas aiheuttaa sen, että sovellus renderöi "UsersByID" komponentin.
  return (
      ...
      <tbody>
        {users.map((results, index) =>
          <tr key={results.id}>
            ...
            <td>
              <Link to={`/users/${results.id}`}>{results.name}</Link>
            </td>
