...
import { createNewCommentValue, likeValueButton, deleteValueButton } from '../reducers/blogReducer' // Komponentti ottaa "createNewCommentValue", "createNewValue" ja "deleteValueButton" funktiot käyttöönsä, joka sijaitsee => "blogReducer.js" tiedostossa.
...
const BlogByID = ({ getCurrentBlogs, getCurrentUser }) => {
  ...
  const [comment, setComment] = useState([]) // Alustetaan muuttuja "comment" tilaan, joka saa oletuksena tyhjän taulukon arvon.
  ...
  const newCommentValue = (getCurrentBlogID) => {
    event.preventDefault() // The event.preventDefault() method stops the default action of an element from happening.

    // Alustetaan muuttuja "getCurrentComment", joka saa käyttöönsä {...} sisällä olevan objektin.
    const getCurrentComment = {
      message: comment // eli "getCurrentComment.message" on yhtä kuin => "comment" muuttujan arvo.
    }

    // Funktio yrittää suorittaa "try {...}" sisällä olevat asiat, jos tulee virheitä yms. niin sovellus siirtyy => "catch {...}" osuuden pariin.
    try {
      dispatch(createNewCommentValue(getCurrentBlogID, getCurrentComment)) // Sovellus suorittaa funktion "createNewCommentValue(...)", joka saa kyseiset muuttujat parametrin arvoksi.
      dispatch(showNotificationSuccess('You have successfully added new comment for current blog! :)')) // Sovellus suorittaa funktion "showNotificationSuccess(...)", joka saa kyseisen tekstin parametrin arvoksi.
    ...
    } catch (error) { // Jos funktion suorittamisen aikana tulee mahd. virheitä, niin sovellus suorittaa {...} sisällä olevat asiat.
      dispatch(showNotificationError('There was a problem while adding new comment. Please try again later! :)')) // Sovellus suorittaa funktion "showNotificationError(...)", joka saa kyseisen tekstin parametrin arvoksi.
      setTimeout(() => { // Ajastin, joka muuttaa {...} sisällä olevien muuttujien arvot takaisin alkuperäiseen arvoon eli "null".
        console.log('Notification is now hidden from the user! :)') // Tulostetaan kyseinen teksti konsoliin näkyviin.
        dispatch(hideNotification()) // Sovellus suorittaa funktion "hideNotification()", joka piilottaa "popup" viestin pois näkyviltä.
      }, 3000) // Yllä olevat funktiot suoritetaan 3. sek kuluttua.
  ...
  // Alustetaan muuttuja "searchBlogComments", joka suorittaa {...} sisällä olevat asiat aina,
  // kun kyseiseen funktioon tehdään viittaus. Funktion tarkoituksena on toimia niin, että
  // kun käyttäjä siirtyy tiettyyn blogiin, niin se yhteydessä näytetään vielä muiden tietojen
  // lisäksi kommentit, jotka viittaavaat kyseiseen blogin arvoon.
  const searchBlogComments = () => {

    // Jos alla oleva if-ehto toteutuu eli kyseisellä blogin arvolla ei ole yhtään
    // kommenttia tietokannassa, niin sovellus renderöi {...} sisällä olevat asiat.
    if (!getCurrentBlogData.comments[0]) {
      return (
        <div>
          <ul className='list-group list-group-flush'>
            <li className='list-group-item'>Current blog article does not have any comments on the database! :(</li>
          </ul>
        </div>
      )
    }

    // Muussa tapauksessa sovellus renderöi (...) sisällä olevat asiat takaisin
    // käyttäjälle näkyviin, josta jokainen kommentti on eriteltynä omalle riville.
    return (
      <div>
        <ul className='list-group list-group-flush'>
          {getCurrentBlogData.comments.map(results =>
            <li key={results.id} className='list-group-item'>{results.message}</li>
          )}
        </ul>
      </div>
    )
      ...
      <div className='container'>
        ...
        <div className='mt-5'>
          <div className='input-group mb-3'>
            ...
            />
            <button type='button' className='btn btn-outline-secondary' id='button-addon2' onClick={() => newCommentValue(getCurrentBlogData.id)}>Add</button>
          </div>
          <div>
            <h3>Current blog article has following comments:</h3>
            {searchBlogComments()}
          </div>
        </div>
      </div>
