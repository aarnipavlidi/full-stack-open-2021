...
const blogReducer = (state = [], action) => {
  ...
  // Jos alla oleva if-ehto toteutuu, niin sovellus suoritta {...} sisällä olevat asiat eli
  // aina kun käyttäjä haluaa lisätä uuden kommentin tietokantaan, niin alla oleva funktio
  // suoritetaan, muutetaan "state":n tilaa kopioimalla alkuperäinen data, johon lisätään
  // uusi data mukaan.
  if (action.type === 'ADD_NEW_COMMENT') {

    const getCurrentID = action.data.blog // Alustetaan muuttuja "getCurrentID", joka on yhtä kuin kyseinen muuttujan arvo.
    ...
    // "getCurrentID" muuttujan kanssa.
    const getMatchingID = state.find(results => results.id === getCurrentID)

    // Alustetaan muuttuja "addNewValue", jossa siis kopioidaan "getMatchingID"
    // sisällä oleva data, jonka sisältä muokataan "comments" objektia => tekemällä
    // uusi kopio => "...getMatchingID.comments" ja lisäämällä perään uusi data.
    const addNewValue = {
      ...getMatchingID,
      comments: [...getMatchingID.comments, action.data]
    }
    ...
    // alkuperäisen muuttujan arvot. Muussa tapauksessa renderöidään "addNewValue" arvot.
    return state.map(results =>
      results.id !== getCurrentID ? results : addNewValue)
  }
...
// Viedään muuttujan "createNewCommentValue" sisältö käytettäväksi, jotta esim. "index.js"
// tiedosto pystyy hyödyntämään sovelluksen aikana. Aina kun kyseiseen funktioon
// tehdään viittuas, niin sovellus tekee {...} sisällä olevat asiat.
export const createNewCommentValue = (getCurrentBlogID, getCurrentBlogComment) => {
  return async dispatch => {
    const response = await blogsService.createNewBlogComment(getCurrentBlogID, getCurrentBlogComment)
    dispatch({
