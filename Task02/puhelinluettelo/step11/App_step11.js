...
// Luodaan komponentti "Notification", jonka sisälle luodaan yksi (1) muuttuja => "message" käytettäväksi komponentin sisällä.
// Riviltä 175 löytyy => "<Notification message={statusMessage} />" ja kuten huomataan, niin tämä muuttuja on viitattu olemaan yhtä kuin x muuttujan kanssa.
// Jos tätä yhtä (1) muuttujaa ei olisi laitettu esim. => "const Notification = () => {...}" niin alla oleva komponentti ei toimisi ollenkaan!
const Notification = ({ message }) => {
  if (message === null) { // Jos muuttuja "message" => {statusMessage} on arvoa "null" niin mitään ei renderöidä takaisin käyttäjälle.
    return null
  }

  // Jos muuttuja "message" => {statusMessage} on epätosi arvon "null" kanssa, niin renderöidään alla oleva asia.
...
const App = () => {
    ...
    if(persons.some(personsSearch => personsSearch.name === newName)) {
      if(window.confirm(`${nameObject.name} is already added to the records. Do you want to update the old number with a new one?`)) {
        notesService // Käytetään muuttujan "notesService" moduulin sisältöä, josta löytyy muuttuja => "const updateValue = (id, changeValue) =>".
          ...
          setStatusMessage(` You have updated ${nameObject.name} on the phonebook! `) // Muutetaan "statusMessage" muuttujan arvo kyseiseen tekstiin. Muista, että kyseinen muuttuja on yhtä kuin {message} <=> "Notification" komponentin sisällä oleva muuttuja.
        ...
        setStatusMessage(` You have added ${nameObject.name} to the phonebook! `) // Muutetaan "statusMessage" muuttujan arvo kyseiseen tekstiin. Muista, että kyseinen muuttuja on yhtä kuin {message} <=> "Notification" komponentin sisällä oleva muuttuja.
        setTimeout(() => { // Käytetään "setTimeOut()" funktiota, jotta saadaan ilmoitus poistumaan tietyn ajan päästä eli tässä tapauksessa ilmoitus poistuu näytöltä 2. sekunnin (eli 2000 ms) kuluttua.
          setStatusMessage(null) // Muutetaan "statusMessage" muuttujan arvoksi => "null". Muista, että kyseinen muuttuja on yhtä kuin {message} <=> "Notification" komponentin sisällä oleva muuttuja.
        }, 2000)
      }
  ...
  const handlePersonDelete = (event) => {
    ...
    if (window.confirm(`Are you sure you want to delete ${persons[getIndex].name} from the records?`)) {
      ...
      setStatusMessage(` You have deleted ${persons[getIndex].name} from the phonebook! `) // Muutetaan "statusMessage" muuttujan arvo kyseiseen tekstiin. Muista, että kyseinen muuttuja on yhtä kuin {message} <=> "Notification" komponentin sisällä oleva muuttuja.
      setTimeout(() => { // Käytetään "setTimeOut()" funktiota, jotta saadaan ilmoitus poistumaan tietyn ajan päästä eli tässä tapauksessa ilmoitus poistuu näytöltä 2. sekunnin (eli 2000 ms) kuluttua.
        setStatusMessage(null) // Muutetaan "statusMessage" muuttujan arvoksi => "null". Muista, että kyseinen muuttuja on yhtä kuin {message} <=> "Notification" komponentin sisällä oleva muuttuja.
      }, 2000)
    }
