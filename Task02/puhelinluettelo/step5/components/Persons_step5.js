  ...
  return(
    <ul>
      {props.personValue.filter(a => a.name.toLowerCase().includes(props.filterNameValue.toLowerCase())).map(b =>
        <li key={b.name}>Name: {b.name}, {b.phonenumber} (phone number)</li> // {b.name} + {b.phonenumber} = hakee arvot muuttujasta "persons" ja näyttää "name" sekä "phonenumber" arvot käyttäjälle.
      )}
    </ul>
  )
}
