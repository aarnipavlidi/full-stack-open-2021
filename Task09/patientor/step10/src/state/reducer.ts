...
// Lisätty "9.24: patientor, step9" tehtävää varten alla oleva muuttuja eli
// "addPatientEntryTolist", joka suorittaa {...} sisällä olevat asiat aina
// kun kyseiseen funktioon tehdään viittaus. Funktio saa käyttöönsä "getData"
// parametrin, jolle annetaan tyypiksi "Patient". Tämä johtuu siitä, koska
// kun uusi "entry" arvo lisätään tietokantaan, niin se palauttaa meille takaisin
// datan, joka noudattaa "Patient" tyyppiä. Funktiolle annetaan objektin arvo
// "newEntry", johon funktion kautta tuleva data sijoitetaan.
export const addPatientEntryTolist = (getData: Patient): Action => {
  return {
...
// "9.24: patientor, step9" tehtävää varten tehdyt muokkaukset:
// Muokattu alla olevaa "Action" tyyppiä niin, että lisätty
// uusi "action" nimeltään "ADD_PATIENT_ENTRY", jonka data
// noudattaa "Patient" tyyppiä (eli potilaan kaikki mahdolliset
// objektien arvot mitä potilaalla voi olla).
...
export type Action =
  ...
  | {
      type: "ADD_PATIENT_ENTRY"; // Lisätty kyseinen objekti => "9.24: patientor, step9" tehtävää varten.
      newEntry: Patient; // Lisätty kyseinen objekti => "9.24: patientor, step9" tehtävää varten.
    };
...
export const reducer = (state: State, action: Action): State => {
  switch (action.type) {
    ...
    // Lisätty "9.24: patientor, step9" tehtää varten alla oleva "case" eli
    // "ADD_PATIENT_ENTRY", joka toimii samalla periaattella kuin yllä oleva
    // "ADD_PATIENT". Ainoa ero on vain ainoastaanm, että palvelimen kautta
    // tuleva data tallennetaan "stateen" => "personalPatientData" objektin alle.
    case "ADD_PATIENT_ENTRY":
      return {
        ...state,
        personalPatientData: {
          ...state.personalPatientData,
          [action.newEntry.id]: action.newEntry
        }
      };
    default:
      return state;
