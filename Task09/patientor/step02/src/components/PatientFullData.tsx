...
import { useStateValue } from "../state"; // Tuodaan "useStateValue" niminen muuttuja => "state.tsx" nimisen tiedoston kautta sovelluksen käytettäväksi.
    ...
    const getPatientPersonalData = async () => {
      try { // Ensin yritetään suorittaa "try" osuus, jos sen aikana tulee virheitä, niin sovellus suorittaa "catch" osuuden.
        ...
        if (checkingPatientData === true) {
          ...
          dispatch({ type: "GET_PATIENT_PERSONAL_DATA", payload: patientPersonalDataFromDatabase });
        }
